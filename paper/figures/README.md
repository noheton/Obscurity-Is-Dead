...
have mixed provenance: `logo-obscurity-is-dead.png` (the shattered-jar
hero) was generated by **Google Gemini** at the author's request on
2026-05-02 and is the final deliverable for that asset;
`logo-pandora-jar-intact.png` (the intact-jar companion that anchors
§10) is **still the AI-authored typographic placeholder** rendered by
`logo-placeholders.py`, pending the second Gemini deliverable. The
intact-jar placeholder will be swapped for the final Gemini PNG when
the human author supplies it; until then the placeholder identity is
visible in the rendered PDF (rule 1). Neither logo is part of the
...
| `logo-pandora-jar-intact.png` | `fig:pandora-jar-intact` | The proverbial jar — intact (companion to the shattered logo, anchors §10) | **AI-authored placeholder** *(PENDING-GEMINI-FINAL)* — currently rendered by `logo-placeholders.py` (matplotlib typographic placeholder, 950x944 RGBA, ~83 kB); to be replaced with the second Google-Gemini deliverable when the human author supplies it. Per rule 1, the placeholder identity is visible in the rendered PDF and in §10 prose at `main.tex:2138-2149`. Confirmed 2026-05-03 illustrator pass: placeholder still in place; binary not regenerated this pass (script unchanged). |
