...
Authorship: AI-authored (illustration agent run, prompt
`docs/prompts/illustration-prompt.md`, session
claude/run-illustration-agent-IRJKO, 2026-05-02).
...
def box(x, y, w, h, label, sub=None, fc="#ffffff", ec=dlr_style.DLR_GRAY,
        lw=1.4, fontsize=9.5, weight="normal"):
    ...
    if sub:
        ax.text(x + w / 2, y + h / 2 - 0.22, sub, ha="center", va="center",
                fontsize=8, color=dlr_style.DLR_GRAY)
    return (x + w / 2, y + h / 2)
...
def arrow(p1, p2, label=None, color="#444", style="-|>", lw=1.5,
          rad=0.0, label_offset=(0, 0.18), label_color="#222",
          label_size=8):
...
arrow(app_c,    s_legacy[0:2], label="USES (undocumented)",
      color="#c0392b", lw=2.0, label_color="#c0392b")
...
arrow(ha_c,      s_legacy[0:2], label="SELECTS\n(apk.md line 52)",
      color="#b8860b", lw=2.0, rad=-0.25, label_color="#b8860b",
      label_offset=(0, -0.05))
...
ax.text(5.0, 6.15, "EcoFlow PowerOcean — three API surfaces and how they are reached",
...
ax.text(5.0, 0.18,
        ...
        ha="center", va="center", fontsize=8.5, color=dlr_style.DLR_GRAY,
        style="italic")
