...
A short methodological note from me (Florian Krebs), placed up-front because *how* you read this paper matters as much as *what* is in it. Treat the note as an invitation — to read closely, to argue back, and to bring evidence. The framings below are the lenses I would most want a thoughtful reader to bring to the rest of the document; none of them softens an empirical claim, and all of them open a door to discussion.

*From a horticulture controller to AI safety.* This paper began as a pure interoperability project — a few weekends of trying to bring a Spider Farmer BLE controller and an EcoFlow PowerOcean energy system into Home Assistant without depending on the vendor cloud. AI assistance turned what would have been months of solo reverse engineering into days, and the integrations work. But the *speed* of that collapse — the disappearance of the effort gap that has historically protected proprietary protocols — kept surfacing harder questions: if I can do this in days as a hobbyist, what does the same compression look like for an attacker, a paper mill, or an unverified claim that nobody noticed an LLM wrote? The case studies stayed; the centre of gravity shifted. What started as *"how do I make this controller talk to my smart home"* became *"what does responsible practice look like when the methodology that produced this paper is the same methodology that, used badly, fills the literature with synthetic noise (§7.6, §7.7)?"* That arc is the reason the paper has a §10 at all; without it, this would have been a how-to and not a piece of empirical methodology.

*Process, not product.* View this work less as the *output* of frontier agentic AI and more as a *process*. The methodology here tries to leverage generative AI's analytical lift while warding against its documented failure modes — fabrication, opacity, dual-use amplification, and the asymmetric collapse developed in §7.3 and §7.7. The case-study substance is mostly prior community work; the contribution is the eight-practice register in §10 and the discipline that produced it. If the discipline is the contribution, then disagreement *about* the discipline is the most useful kind of engagement a reader can offer, and it is the kind I am most hoping for.

*Frameworks evolve with capabilities.* The eight practices in §10 are calibrated to a specific snapshot — Claude Opus 4.7, the Claude Code CLI, the Consensus literature front end, and consumer-IoT case studies in mid-2026. That snapshot will age. Some practices will be subsumed by tooling (transcript export becomes a click); some will become inadequate (verification-status legends will need finer grain); some I cannot yet name will need to be invented. Treat the eight as a starting point and a conversation opener, not a final state, and please tell me when you see one of the eight begin to age out.

*Pandora's jar is open.* The shattered-jar logo in the front matter is deliberate, and the framing is firm: the empirical literature on fabricated citations [L-SLOP-1, L-SLOP-2], paper-mill output [L-SLOP-8], and longer-horizon model collapse [L-MC-1, L-MC-3] documents what is already out of the jar, and no appeal to obscurity, vendor terms of service, or moratoria on capability research will put it back. The intact-jar reference in §10 is the Hesiodic counterpoint: the jar can be closed only if Hope is what remains inside, and Hope here is operational — structured, transparent, attributed practice that the next reader can audit, replicate, or refute. The point of the framing is not pessimism; it is to name what we are working with so the work has somewhere honest to go.

*The irony, acknowledged.* I am aware of the obvious irony of writing a paper *about* the failure modes of AI-assisted research *using* AI-assisted research. I do not have a clever way out of it, and pretending I did would itself be one of the failure modes. The honest answer is the one operationalised in §9: AI usage is disclosed at the *artifact* level, every claim is backed by an exportable transcript committed in this repository, and the methodology tries to make the discipline that — absent disclosure — would let the failure modes scale. Refusing to use the tools while writing about them would not have produced a more honest paper; it would have ceded the methodological ground to those who use them without disclosure (§7.6, §7.7). Pandora's jar is open. The only way out is through, and "through" looks like this — disclosed, attributed, auditable, and willing to be argued with.

*On the unsaid accusation of cheating.* A second strand of skepticism, often unsaid, is the suspicion that AI-assisted authorship is a quieter form of plagiarism — that the methodology I am defending here is, on closer reading, an elaborate way to launder synthetic prose into the scientific record. I take that suspicion seriously and I think it is largely correct *about the prevailing baseline* (§7.6 documents what undisclosed AI use looks like at scale; the empirical fabrication-rate literature in §9.4 is what concealment produces). What I am trying to construct in this paper is the alternative the suspicion does not yet have: a discipline under which AI-assisted research is inspectable on its own terms, transcript by transcript, citation by verified citation. The eight practices in §10 are framed in §8.3 as exactly this — a *condition catalogue* that converts a legitimate objection into a checkable one. If you arrived as a skeptic, the most useful thing you can do with this paper is to argue with the catalogue: which row is missing, which row is insufficient, which row will the next tooling cycle subsume. That is the conversation I am writing for.

*What surprised me about the assistant.* Working through the case studies, I expected — and braced for — moments where the model would refuse to help me reverse-engineer a proprietary BLE protocol or pull credentials out of a vendored APK, or where the session would be flagged or terminated. That happened far less often than I anticipated. The dominant register was *cooperative competence*, including for steps that an attacker would also want. This is a useful empirical observation, and a sobering one: it tells us that current guardrails are real but bounded, and that the surface they protect is much smaller than the surface they touch. The conclusion I take from this is uncomfortable but not pessimistic: guardrails are a band-aid — necessary, but a band-aid (§7.4 develops this against the Mythos / Project Glasswing case). The durable answer is *security by design at the artifact level* — published specifications, authenticated and scoped APIs, and threat models that assume the existence of capable, willing, AI-augmented adversaries (Figures 6 and 7). Treating AI as just another vector under-counts what is changing; the magnitude and frequency of attempts that any given system will face is materially higher in 2026 than it was in 2024, and the trajectory points further the same way. Build for that.

*Open to constructive criticism — actively.* I welcome criticism, replication attempts, and challenges to the framework — to the eight practices, the case-study choices, and the legal framing in §9. Disagree with the synthesis; surface a counter-example; flag a fabricated citation, an unsourced legal claim, or a verification-status entry that cannot survive scrutiny — all of these are useful, and all of them are welcome. The transcript-as-artifact discipline (rule 4 in `CLAUDE_CODE_INSTRUCTIONS.md`) is what makes serious critique tractable: every claim in this paper is backed by an exportable conversation log committed in this repository, so an objection can land on a specific line, transcript, or commit rather than on vibes. If you find such a claim that cannot survive scrutiny, or if you simply have a better way to do any part of this, I genuinely want to hear it.

*Not a paper mill — a shower-thought-to-plausibility loop.* This needs saying clearly because the empirical baseline is bad: paper-mill output and undisclosed AI-assisted production are the dominant failure modes [L-SLOP-8, L-SLOP-1, L-SLOP-2]. The methodology described here is designed to do the opposite. The agent pipeline (`docs/prompts/`) takes a *hypothesis* — often a "shower thought" provoked by a specific device, transcript, or paragraph — and iterates it through structured passes (research → source analysis → scientific writing → illustration → layout / readability scrutiny). At each stage the question is the same: *does this still look plausible against the evidence we now have?* Plausible hypotheses survive into the paper with their verification status visible; implausible ones are rejected, recorded in the logbook with a reason, and remain auditable. The output volume is therefore intentionally bounded by what survives plausibility, not by what an LLM can be cajoled into producing. The repository carries roughly as many lines of methodological scaffolding as paper prose precisely so the volume cannot run away from the verification.

*An invitation, in concrete form.* GitHub issues on this repository are a first-class channel for new ideas, counter-examples, and critique. Issues labelled `idea` (a hypothesis worth investigating), `critique` (a specific challenge to a claim, figure, or framing), or `provenance-gap` (a missing transcript or unverifiable artifact) feed directly into the agent pipeline. An `idea` triggers a research-protocol pass; a `critique` routes to the relevant scrutinizer hand-back; a `provenance-gap` routes to the meta-process case study (§5). If you have read this far, you are already the kind of reader I am writing for — and the issue tracker is the best place to continue the conversation.
...
The effort-gap collapse documented by this paper is software-side: it concerns the analysis of mobile companion apps, cloud APIs, and BLE/MQTT protocols where the obscurity barrier is a labour cost imposed on a researcher who already has root-equivalent access to the artefacts. A parallel, *researcher-hypothesised and AI-assisted-research-confirmed* extension of the same thesis applies to the **hardware-side effort gap** — the cost of physically extracting firmware, locating debug interfaces, and mounting side-channel or fault-injection attacks on commodity embedded devices.

Two peer-reviewed quantitative anchors triangulate this hypothesis. The ChipWhisperer platform documents an approximately 100× equipment-cost reduction relative to prior commercial side-channel rigs [L-HW-RE-2][^hwre-cluster]. Vasile, Oswald & Chothia's systematic survey of 24 commercial IoT devices reports that an exposed UART suffices for firmware extraction in more than 45% of cases [@vasile2018breakingallthethings].

A SOUPS 2020 cognitive-process study finds that students reach intermediate hardware-reverse-engineering proficiency in 14 weeks of structured training, quantifying skill-floor compression [@becker2020hwreexploratory]. A peer-reviewed embedded-systems attack taxonomy (Papp, Ma & Buttyán, 2015) [@papp2015embedded] complements the cost- and skill-floor anchors at the categorisation layer.

Practitioner handbooks bookend the period: the 2003–2004 Grand / Huang baseline (bespoke equipment, multi-day soldering, expert-only skill floor) and the 2022 van Woudenberg & O'Flynn handbook (single volume + ~USD 300 toolchain) [L-HW-RE-5][^hwre-cluster]. AI-assisted PCB component identification has matured; full netlist reconstruction is emerging [@botero2021hwretutorial]; and JTAGulator-class commodity devices, documented in grey literature, automate debug-interface discovery on unknown PCBs [@grand2013jtagulator].

The hardware-side cluster is presented as a *triangulated practitioner observation*, not as a benchmarked finding equivalent to the software-side anchors; the asymmetry between the two evidence bases is itself a finding of this paper and is treated explicitly in §6.8.
...
- Migration framework `async_migrate_entry` in `__init__.py` line 95; the integration is now at `VERSION = 3` past the T4-era `1→2` migration. The `2→3` step is **not covered by any preserved chat transcript**, but the migration logic itself — vendored at `experiments/spider-farmer/original/__init__.py` lines 95–135 — is self-documenting and reconstructs to four steps:
    1. Drop the legacy MQTT-only config-entry fields `uid` and `mqtt_topic`.
    2. Derive `pid` deterministically from the stored BLE address (failing closed if no BLE address is present).
    3. Idempotently carry forward the `v1→v2` CB-key correction.
    4. Bump the entry version and emit `"Migrated Spider Farmer entry %s to version 3 (BLE-only)"`.

    Architecturally this is a transport simplification (BLE-only; MQTT retained only as a security-research dataset surface, §3.6), not a protocol change. **Provenance gap that remains:** the upstream commit history of `noheton/spider_farmer` was not retrievable from the build environment used for this paper (clone, GitHub API, and codeload tarball all failed); reconstruction worked from the embedded `original/` snapshot at repo commit `ffdf60c` only. Date, commit SHA, and any associated PR or issue thread for the v2→v3 transition are therefore unverified-external.
...
As of 2026-05-03 the upstream `noheton/powerocean-dev` repository has removed the APK splits, derivative APK analysis, vendor PDFs, vendor sample-code archive, GDPR-personal-data log files, and the `equipment.md` personal-data file from its working tree (commit `5c8b815cf9` [@noheton2026powerocean5c8b815]), citing UrhG and GDPR concerns. The accompanying `doc/README.md` [@noheton2026powerocean_docreadme] explicitly notes that `git rm` does not purge prior commits from repository history (a fresh-clone `git filter-repo` rewrite would be required for a true purge). The snapshot preserved here under `experiments/ecoflow-powerocean/original/` predates that redaction event and is retained for research provenance only.
...
1. **APK string and action-field extraction** to enumerate writeable parameters.[^ef-twotrack]

[^ef-twotrack]: This describes the *researcher-side* method used in this case study. The *upstream-redistributable* method is now framed differently: the upstream `noheton/powerocean-dev` `DISCLAIMER.md` (added 2026-05-03) [@noheton2026powerocean_disclaimer] documents the integration's committed code as derived from observed HTTP traffic between the EcoFlow mobile app and the EcoFlow cloud API, citing § 69e UrhG and Art. 6 of EU Software Directive 2009/24/EC. Commit `1aa96507ef` [@noheton2026powerocean1aa9650] separately captures OCPP-1.6 schema material from an APK decompile, indicating a two-track methodology in which decompilation supports protocol *understanding* while the redistributable artifact contains only traffic-observable structures. The paper's case study documents the research arm; the upstream disclaimer describes the redistribution arm.
...
A subsequent upstream development surface — the OCPP backend-binding work landed on `noheton/powerocean-dev` between 2026-05-01 and 2026-05-03 (~+440 LOC across `api.py`, `__init__.py`, `services.yaml`, and `const.py`; new endpoints `GET /provider-service/app/ocppPlatformConfig/list` and `POST /provider-service/app/ocppPlatformConfig`) — illustrates a concrete *catalog-vs-runtime* asymmetry inside the same legacy bearer-token API plane: the integration can now write the EcoFlow-side OCPP catalog, but the upstream `README.md` notes a runtime-handover gap, namely a `vendorInfoSet` proto write that is *not yet shipped* and that the charger requires before it will redirect to the third-party host at runtime [@noheton2026powerocean5c8b815]. Catalog-level interoperability is therefore reachable while runtime-level interoperability is not — a useful empirical data point for the synthesis on the maturity of effort-gap compression (§6).
...
- **Live-credential leakage.** `docs/sources.md` S-SF-5 carried the recovered MQTT credentials for the Spider Farmer broker. These have been replaced with `[REDACTED]` markers in all researcher-authored files per `docs/redaction-policy.md` R-SF-1..R-SF-2, and the raw values were excised from git history on 2026-05-04 by a `git filter-repo --replace-text` pass against the catalogue in `docs/git-history-rewrite-plan.md`; the rewrite tip carries the annotated tag `pre-publication-clean`. Vendored binary archives in `experiments/spider-farmer/original/doc/` retain the strings inside their packed entries because `--replace-text` does not descend into binary blobs; the policy treats them as a public-mirror cut-over decision (drop the archives or ship them with a caveat) rather than a residual leak.
...
| Primary AI lift | Manifest-permission audit; existing-solutions enumeration; technique-inventory bootstrap from prior reports | Static APK analysis (unzip + strings + grep); cross-implementation validation against ES-6 (`[REDACTED:repo-path:BALBOA-UPSTREAM-2]`); identification of endpoints not in any open-source library |
...
The §1.4 thesis is supported on the software side by a peer-reviewed quantitative base (cluster A in `docs/sources.md`, entries L-RE-1 through L-RE-8) that includes a 24.4% cognitive-burden reduction in AI-assisted decompilation, a 31%→53% correct-solve rate uplift on reverse-engineering tasks, and a >100% re-executability rate for AI-assisted code synthesis. The hardware-side extension introduced in §1.4 rests on a structurally different evidence base.

The single peer-reviewed equipment-cost anchor is the ChipWhisperer 2014 platform's ~100× reduction (L-HW-RE-2[^hwre-cluster]). The single peer-reviewed survey datapoint is Vasile, Oswald & Chothia (2018) [@vasile2018breakingallthethings]: an exposed UART suffices for firmware extraction in more than 45% of 24 commercial IoT devices. The skill-floor anchor is Becker et al. (2020, SOUPS) [@becker2020hwreexploratory]: students reach intermediate proficiency in 14 weeks of structured training. The attack-taxonomy complement is Papp, Ma & Buttyán (2015, IEEE PST) [@papp2015embedded], applying a five-dimension taxonomy to a CVE subset.

Practitioner handbooks bookend the period (Grand 2004 / Huang 2003 → van Woudenberg & O'Flynn 2022, L-HW-RE-5)[^hwre-cluster]; JTAGulator-class commodity tooling supplements the cluster as grey literature [@grand2013jtagulator], and an emerging AI-assisted PCB-RE literature reports its strongest results at the component-detection layer rather than at full netlist reconstruction [@botero2021hwretutorial].
...
A live counter-data-point landed during the writing of this paper. On 2026-04-07 Anthropic announced **Claude Mythos Preview** [@anthropic2026glasswing; @anthropicred2026mythos] — a frontier model that, in the company's own published red-team work, autonomously identified zero-day vulnerabilities in *every major operating system and every major web browser*.

Specifically: the oldest such finding is a 27-year-old OpenBSD bug; Mythos chained four-vulnerability browser exploits with JIT heap-spray, autonomously obtained local privilege escalation via subtle race conditions and KASLR bypass, and split a multi-gadget ROP chain across packets to root unauthenticated NFS servers. Crucially for this paper's thesis, it also reverse-engineers stripped closed-source binaries to plausible source code at scale — the strongest published acknowledgement to date that the offensive-side gap is closing faster than the L-VD-1 / L-VD-2 era of evidence (which the §1.4 framing was calibrated against) suggested.

Mythos is not yet broadly deployed adversarial capability. Anthropic chose *not* to release it generally, instead launching **Project Glasswing**, a coordinated defensive deployment to AWS, Apple, Cisco, CrowdStrike, Google, JPMorganChase, Linux Foundation, Microsoft, NVIDIA, and Palo Alto Networks. Anthropic shipped the more cautious Claude Opus 4.7 [@anthropic2026opus47] as the safeguarded sibling. But the existence proof is now concrete: capable autonomous offensive AI is on hand at a frontier lab, and the lab itself describes it as a *"watershed"* demanding industry-coordinated defence. The asymmetry-of-collapse argument in this section is not falsified — verifying an integration is still cheaper than verifying an exploit — but the gap is narrower, and the time horizon over which it remains a defender-favouring asymmetry is shorter, than this paper's first draft assumed.
...
**Guardrails as band-aid: a security-by-design imperative.** The Project Glasswing model (§7.3) and the safeguarded Claude Opus 4.7 release [@anthropic2026opus47] are interesting precisely as a *concrete public example of guardrails as band-aid*. Anthropic's response to demonstrating Mythos's offensive capability was not to install guardrails sufficient to release Mythos broadly — they explicitly chose not to release it generally, and the safeguards bolted onto Opus 4.7 are described as "automatically detect and block requests that indicate prohibited or high-risk cybersecurity uses". This is necessary work, and Anthropic is being unusually transparent about it. But the band-aid is structural: every guardrail is a classifier, every classifier has a false-negative rate, and any false-negative rate combined with autonomous offensive capability and global proliferation *plausibly* converges to attacker-side capability over a horizon whose length we cannot yet quantify — a claim we present as engineering intuition rather than proof, and one the L-MYTHOS evidence base [@anthropic2026glasswing; @anthropicred2026mythos] documents qualitatively but does not yet bound numerically. The author's own observation while writing this paper (Author's Note: *What surprised me about the assistant*) corroborates this from the inside: Opus 4.7 — the safeguarded sibling — was *cooperatively competent* at the steps an attacker would also want, and the session was not flagged. The durable answer is *security by design at the artifact level* — published specifications, authenticated and scoped APIs, and threat models that assume the existence of capable, willing, AI-augmented adversaries (Figures 6 and 7). The frame to retire is "AI is just another vector"; the frame to adopt is *AI massively increases the magnitude and frequency of attempts that any given system will face, and only systems that survive that increase by design will survive at all*.
...
- Buchanan, Hill & Shapoval [@buchanan2024fabricated], *The American Economist*, replicate the fabrication finding (>30% non-existent ChatGPT citations) in a third discipline, with reliability decreasing as prompts become more specific.
- Suchak et al. [@suchak2025nhanes], *PLOS Biology*, document a real-world consequence: NHANES-based formulaic single-factor papers grew from 4 per year (2014–2021) to **190 in the first ten months of 2024** — direct empirical evidence of paper-mill / AI-assisted output flooding the literature.
- Kendall & Teixeira da Silva (2024), *Learned Publishing*, and Sabel & Larhammar [@sabel2025stockholm], the Stockholm Declaration, frame this at the system level: paper mills are now AI-assisted at industrial scale, and the publishing model needs structural reform (L-SLOP-5).
...
A second, longer-horizon argument cuts in the opposite direction. Shumailov et al. (2024, *Nature*) showed that **AI models trained recursively on AI-generated content collapse** [@shumailov2024modelcollapse] — the tails of the original distribution disappear and successive generations of models forget the genuine human-data signal. Seddik et al. [@seddik2024collapse] prove this is unavoidable when training is purely synthetic but bounded when synthetic and real data are mixed below a threshold. Gerstgrasser et al. [@gerstgrasser2024accumulate] extend this further: *accumulating* real and synthetic data over time (rather than replacing real with synthetic) can avoid collapse entirely, with a finite test-error upper bound independent of iteration count. Suresh et al. (2024) characterise the *rate* of collapse for fundamental distributions [L-MC-5]; ForTIFAI / Shabgahi et al. (2025) propose confidence-aware training objectives that delay collapse onset by >2.3× [L-MC-2].
...
**The empirical baseline is severe.** Ren et al. [@ren2019iotinforming] is the cornerstone measurement: 81 consumer IoT devices in US/UK labs, 34,586 controlled experiments characterising destinations, encryption state, inferable interactions, and unexpected exposures (including a recording device that covertly transmitted video). Apthorpe et al. (2017) showed that even when the payload *is* encrypted, traffic-rate side channels reveal sensitive user interactions to a passive observer such as the user's ISP for four representative devices (sleep monitor, indoor camera, smart switch, smart speaker) [L-PRIV-2]. Acar et al. [@acar2020wisec] (WiSec 2020) reported >90% accuracy in identifying device states and ongoing user activities from passively-sniffed encrypted traffic across WiFi, ZigBee, and BLE. Apthorpe et al. (2018) surveyed 1,731 US adults across 3,840 contextual-integrity information flows and found that the egress patterns documented in the measurement studies are *contrary to* what users hold as smart-home privacy norms, not just contrary to a regulatory ideal [L-PRIV-4].

**The companion-app surface compounds the cloud surface.** Most consumer IoT devices route a second telemetry path through their mobile companion app. Nan et al. [@nan2023iotapp] statically analysed **6,208** IoT companion apps and identified **1,973** that exposed user data — health status, home address, behavioural patterns — to first or third parties without proper disclosure, covering devices from at least **1,559 distinct vendors**. Tazi et al. (2025) and Neupane et al. (2022) audited 455 IoT companion apps and found systematic over-requesting of permissions unrelated to the apps' stated purpose, with two of the analysed apps transmitting credentials in unencrypted form [L-PRIV-6, L-PRIV-8]. Chatzoglou et al. (2022) confirmed the same pattern across >40 chart-topping Android IoT apps from six device categories [L-PRIV-7].
...
- **Trust laundering through a benign-looking artifact.** A malicious integrator can publish a Home Assistant custom integration that *also* exfiltrates household credentials. The community-trust signal that legitimises an upstream community integration such as `[REDACTED:repo-path:BALBOA-UPSTREAM-1]` or `[REDACTED:repo-path:BALBOA-UPSTREAM-2]` — a working integration plus a non-empty README — is exactly what an adversary needs to forge.
...
### 8.1 A terminological precision before the call-to-action

A persistent source of confusion in the public debate — and, increasingly, in the academic one — is the conflation of *artificial intelligence* in the broad methodological sense (the family of methods spanning symbolic systems, classical machine learning, deep learning, reinforcement learning, and the present generation of large language models) with *generative AI* in the narrow sense (the LLM-and-diffusion-model subset that drives the current hype cycle). The two are not interchangeable. The empirical effort-gap collapse documented in §3 and §4, and the meta-process collapse documented in §5, are driven specifically by the *generative-AI* subset; the broader AI methods family — adversarial-machine-learning detectors for hardcoded secrets, classical static analysers, symbolic execution engines, supervised classifiers for vulnerability triage — has been shaping the security-research workflow for years and will continue to do so after the present hype cycle subsides. Throughout this paper, where the distinction matters, *generative AI* (or *Gen AI* / LLMs) refers to the narrow subset and *AI* refers to the broader methods family. The call-to-action that follows is framed against the broader family on purpose: norms calibrated only to the current LLM cohort will age out of relevance the moment the cohort does, whereas norms calibrated to the *AI methods family as a whole* are the ones we will still need in 2030.

### 8.2 Call-to-action: generate the norms while the practice is young

The empirical moment this paper documents is unusual: we are among the first cohort of researchers to experience AI-assisted scientific work as a routine modality rather than an experimental novelty. That position carries a corresponding obligation — to use the window in which the practice is still being formed to *generate the norms of acceptable use*, rather than to inherit them, retrofit them, or have them imposed after the fact. Even on the conservative reading of the present hype cycle — that current Gen AI capabilities will plateau, that funding will contract, and that a portion of the integration tooling will recede — the underlying AI methods family will not leave the scientific workflow. From the present moment onwards, AI-assisted research is part of how science is produced; the open question is the *discipline* under which it is produced.

We argue that the eight integrated practices developed in §10 — transcript preservation, verification-status labelling, provenance maps, mirror discipline, the recursive meta-process case study, base-rate-anchored AI disclosure, legal honesty about authorship, and FAIR alignment as a precondition — are best understood as a *condition catalogue* for AI-assisted work to be defensible as research. They are not a manifesto; they are a checklist whose items can be argued with row by row. The call-to-action is correspondingly modest: that the community treat methodological norm-setting for AI-assisted research as itself a research activity, conducted now, in the open, and with the same evidentiary discipline we expect of empirical claims.

### 8.3 Integrating the AI-skeptic position: from opponents to co-norm-setters

A second strand of the contemporary debate must be addressed directly rather than dismissed. A widely held position — sometimes voiced as outright opposition to AI in research, sometimes as a quieter accusation that AI-assisted authorship is a form of cheating — points at real harms: undisclosed authorship that crosses into plagiarism (§7.6), the documented degradation of the scientific commons by paper-mill output [@suchak2025nhanes], and the collapsing baseline of trust in academic text when readers cannot tell what a human wrote and what a model produced. We take that position to be *legitimate*, and we read its underlying claim — that the current default of undisclosed AI assistance is corrosive — as substantially correct.

The eight practices are our proposal for the conditions under which the legitimate concern is *answerable* rather than dismissible. Transcript-as-artifact (practice 1) makes the AI's contribution inspectable rather than hidden; the verification-status ladder (practice 2) makes the read-state of every cited source explicit rather than indeterminate; explicit, base-rate-anchored disclosure (practice 6) replaces the "AI was used to assist" boilerplate with an audit trail. Read in this register, the eight practices are an offer: to skeptics for whom undisclosed AI use is the principal harm, here is a discipline under which AI-assisted work can be evaluated on its own terms. The intended move is from opposition to *co-norm-setting* — disagreement about which practices belong on the list, which ones are insufficient, and which ones the next generation of tooling will subsume is precisely the kind of engagement we are most hoping to provoke. The Author's Note above ("*Open to constructive criticism — actively*") is the same invitation in personal-voice form.

### 8.4 A candidate FAIR for AI-Assisted Research extension

The community already has two precedents for adapting the FAIR Guiding Principles to non-data artefact classes. **FAIR4RS** — the FAIR Principles for Research Software, released in 2022 by the FAIR for Research Software Working Group jointly convened by the Research Software Alliance, FORCE11, and the Research Data Alliance — extends FAIR to source code, scripts, computational workflows, and executables created during the research process [@chuehong2022fair4rs]. **FAIR4ML** — the metadata schema developed by the RDA FAIR for Machine Learning Interest Group (schema v0.1.0, 2024) — extends FAIR to machine-learning *models* and *model evaluations*, building on Schema.org and CodeMeta and now adopted in production by services including InesData and MLentory [@rda2024fair4ml]. Neither of these directly addresses what AI-mediated *research processes* produce: exportable conversation transcripts, versioned prompts, tool-and-model version manifests, verification-status ladders, structured redaction policies for credentials and personal data.

We propose, as a target for community refinement rather than as a finished standard, that the eight integrated practices in §10 be developed into a **FAIR for AI-Assisted Research** extension (working name: *FAIR4AI*). A first-cut mapping is: **Findability** — persistent, citable identifiers for AI conversation sessions, alongside the existing data and software identifiers (DOIs, Software Heritage IDs); **Accessibility** — a standardised redaction policy under which credentials, personal data, device serial numbers, and other rule-12 categories can be removed from public-facing artefacts without breaking the audit trail (cf. `docs/redaction-policy.md`); **Interoperability** — a transcript-export format, prompt-version manifest, and verification-status legend that travel with the manuscript across publishing platforms rather than being trapped in a single vendor's chat UI; **Reusability** — exportable transcripts plus versioned prompts plus declared tool and model versions, sufficient for an independent reader to reconstruct *which decision was made under which AI assistance under which discipline*. The repository accompanying this paper already practises de-facto versions of every element on that list (CLAUDE.md rules 4 and 14; the verification-status ladder in `docs/sources.md`; the redaction policy in `docs/redaction-policy.md`). Naming what we are already doing, and inviting the community to refine, replace, or reject the candidate, is itself the call-to-action: norms generated by the practitioners who use the tools, recorded openly, and surrendered to peer scrutiny before they harden into convention. As of 2026-05-04 we have not located an existing community working group convened under this name (`docs/sources.md` L-FAIR-3, status `[needs-research]`); if one is formed, or already exists under an adjacent name, the proposal here defers to it.
...
### 9.2.1 Community-implementer attribution (redaction notice)
Several community maintainers and their public repositories underpin the Spider Farmer, EcoFlow PowerOcean, and Balboa case studies. Their handles and repository paths have been **redacted** in the public-facing surfaces of this paper (replaced with structured markers of the form `[REDACTED:maintainer-handle:<ID>]` / `[REDACTED:repo-path:<ID>]`) because those community implementers face the same § 44b UrhG and vendor-TOS exposure surface that the paper author does, and they have not had the benefit of qualified legal review. This redaction is a *legal-exposure precaution*, not a claim that the implementations are secret or that their contributions are unrecognised: bibliography keys remain stable internal identifiers, and the cross-implementation comparisons in §3 and §6 are unchanged in substance. The full original→marker mapping is recorded in `docs/redaction-policy.md` (community-implementer anonymisation, IDs `SF-IMPL-1..3`, `EF-IMPL-1`, `BALBOA-UPSTREAM-1..2`); resolution requires repository access at the mapping commit and is governed by the same git-history-rewrite protocol that scrubs other rule-12 items before any public mirror.
...
Pandora's jar is open. Generative AI is now a routine part of how scientific manuscripts are drafted, and the empirical literature documents what comes out of the jar: fabricated citation rates of 18%–55% for raw GPT outputs [@walters2023fabricated], hallucination rates up to 91.4% in systematic-review reference generation [@chelli2024hallucination], paper-mill output growing from 4 to 190 NHANES papers in a single year [@suchak2025nhanes], and a longer-horizon model-collapse risk in which the next generation of foundation models is trained on the corrupted outputs of the present one [@shumailov2024modelcollapse; @seddik2024collapse]. These problems are not solved by refusing to use AI; refusal does not put the technology back in the jar, and it concedes the ground of methodological practice to those who *do* use it without disclosure. What is left in the jar, on the Hesiodic reading, is *Hope* — but the kind of Hope that demands work.
...
A *ninth* practice has come into focus during this paper's own preparation, and is logged here for the next iteration of the framework rather than added to Figure 11. It addresses a meta-question that the eight practices above do not directly answer — *how do we safeguard against unintentional plagiarism?* The verification ladder (practice 2) and provenance maps (practice 3) make every cited claim auditable, and the transcript-as-artifact discipline (practice 1) makes the prompt-to-paragraph link explicit; what they do *not* directly catch is a generation-time risk: that an LLM might reproduce passages from its training data closely enough to constitute plagiarism even when neither the human author nor the model intends it.

9. **Verbatim-string scanning against the assistant's likely training corpus.** The mitigation operationalised in this paper is mundane and partial: every paper-ready paragraph is searched against the public web (Exa MCP `web_search_exa`) for suspicious verbatim n-gram hits before commit; matches above a threshold are either replaced with paraphrase plus citation, or wrapped in an inline quotation with attribution. This is a *first cut*. It does not catch paraphrase-level overlap (which requires similarity tooling beyond exact-string search), and it does not catch overlap with closed corpora the assistant was trained on. The honest framing is that the plagiarism-safeguard problem in the AI-assisted-writing era is not yet solved, and that practice 9 will need to evolve with detection tooling as that tooling matures. It is named here so that the *next* iteration of this framework can argue with it, replace it, or tighten it (Author's Note: *Frameworks evolve with capabilities*).

![Figure 11 — Eight integrated practices × three failure-mode axes. See the in-figure legend for the P / S mitigation roles. Intended as the visual abstract for the submission: the methodological novelty is not any single row but the integration — every failure-mode axis is principally addressed by at least one practice and secondarily reinforced by at least one other.](figures/fig11-eight-practices.svg)
...
**A real-world precedent for the redaction discipline.** Two of the eight practices — provenance maps (practice 3) and FAIR alignment (practice 8) — sit alongside an unlisted hygiene step that this paper enforces under `CLAUDE.md` rules 12 and 13: secrets, personal data, and other rule-12 material must be redacted *and* purged from git history before any public-facing release, because `git rm` alone leaves the deleted blobs reachable from prior commits. A recently-dated, publicly-visible enactment of exactly this discipline appeared during this paper's preparation: on 2026-05-03 the upstream `noheton/powerocean-dev` integration removed its `doc/` subtree (APK splits, derivative APK analysis, vendor PDFs, vendor sample-code archive, GDPR-personal-data files, raw extraction logs) on UrhG / GDPR grounds and replaced it with an 80-line `doc/README.md` that explicitly notes "`git rm` does not purge history" and points readers to `git filter-repo` on a fresh clone for a true purge [@noheton2026powerocean5c8b815; @noheton2026powerocean_docreadme]. The precedent strengthens the methodological argument made here without requiring this paper to claim attribution: an independent AI-assisted-integration project, working under separate constraints, arrived at the same redaction-and-history-rewrite step that the paper's rule-12 / rule-13 framing demands.
