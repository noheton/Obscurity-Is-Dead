...
The published literature already documents the per-vendor base rate. Sivakumaran et al. (2023) found **>70% of 17,243 BLE-enabled Android APKs contain at least one security vulnerability** [L-BLE-4]; Nan et al. (2023) statically analysed **6,208 IoT companion apps** and identified **1,973** that exposed user data, covering devices from at least **1,559 distinct vendors** [L-PRIV-5]. Both studies were *human-led* static analyses with substantial team-years of effort. AI assistance compresses the per-APK cost into a regime where the limit is no longer analyst time but storage, bandwidth, and rate-limit politeness toward the host repository. We enumerate the new threat surface:
