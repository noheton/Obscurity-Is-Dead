...
![The proverbial jar — intact. The companion image to the project logo (`paper/figures/logo-obscurity-is-dead.png`, the shattered Pandora-jar motif used at the front matter): here the jar is whole and closed, with Hope still under its rim. The intact-jar visual is the Hesiodic counterpoint to the shattered-jar logo — the methodology of §10 is the latch that lets *Hope* remain inside the jar even as the rest of its contents have escaped. The asset shown here is currently an AI-authored *placeholder* generated by `paper/figures/logo-placeholders.py`; the final Google-Gemini-generated artwork (commissioned at the author's request, 2026-05-02) replaces this placeholder in place once delivered (rule 1: AI contributions visibly labelled).](figures/logo-pandora-jar-intact.png)
