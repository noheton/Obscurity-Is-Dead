...
\fcolorbox{red}{red!8}{\parbox{0.92\linewidth}{\centering
  \textcolor{red}{\textbf{Condensed venue artifact --- DRAFT.}}\\
  Derivative of the long-form \texttt{paper/main.md} and \texttt{paper/main.tex}
  in the companion repository. Where it omits, it points back to a specific
  section of the long form. It does not introduce new claims. Do not
  redistribute, cite, or upload to a public archive without explicit written
  consent from the author (rule~13, \texttt{CLAUDE.md}).
}}
\end{center}
...
\begin{table}[ht]
  \centering
  \small
  \caption{Headline KPI table for the three primary cases. Source:
    \texttt{paper/main.md} \S 3.7, \S 4.7, \S 5.7; cross-validation rows
    in \S 6.5. Reproduced from the project README.}
...
and provenance-map shape as the device cases. Approximately seventeen and
a half hours of AI-assisted work produced the publication-quality
dual-format manuscript, the seventy-entry literature register, the FAIR
metadata, and the redaction policy that govern this paper. \emph{Full
recursive treatment, transcript register, and dual-use evaluation --- see
\texttt{paper/main.md} \S 5.}
...
case, against the rebuilt PDF and the rule-11 mirror parity check. A
...
cited source: an entry progresses from \texttt{[needs-research]} through
\texttt{[lit-retrieved]} to \texttt{[ai-confirmed]} and finally to
...
FAIR predates LLMs --- and the contribution is the integration. Read as a
single discipline, the eight cover three failure-mode axes that the
literature makes load-bearing. \emph{Sloppification} is the proximate
corruption of the scientific record by undisclosed AI-assisted output;
the verification-status ladder, transcript-as-artifact discipline, and
base-rate-anchored disclosure address it head-on. \emph{Model collapse}
--- the longer-horizon dilution of the corpus on which future foundation
models are trained, formally argued by Shumailov et
al.~\citep{shumailov2024modelcollapse} --- is the same problem at a
...
redaction discipline, and legal honesty about authorship operationalise
the discipline that pays for the methodology's openness honestly.
\emph{Full eight-practice enumeration with worked examples --- see
\texttt{paper/main.md} \S 10.}
...
(L-VD-5). On 7 April 2026 Anthropic disclosed Claude Mythos Preview, a
...
AI-Assisted Research} extension under the working name \emph{FAIR4AI},
...
(rule-12 redaction policy, dual-use mirror per weakness-table row,
configuration-only Phase 3 outcomes, no public push or Zenodo deposit
without explicit researcher consent under rule 13) raise the cost of
...
most hoping to provoke. The candidate FAIR4AI extension above is the
...
\section*{Statement of independence and personal capacity (verbatim from \texttt{paper/main.md} \S 9.5)}
...
\section*{Pointers into the long-form companion repository}
\label{sec:pointers}

The condensed paper deliberately omits, in favour of the long form, the
full eight-practice enumeration with worked examples
(\texttt{paper/main.md} \S 10), the transcript registry and provenance
matrices (\texttt{paper/main.md} \S 3.2 and \S 5.1;
\texttt{experiments/*/provenance.md}), the verification-status ladder
definition and the \texttt{[ai-confirmed]} / \texttt{[lit-read]}
carve-outs (\texttt{paper/main.md} \S 5 and \S 6.4), the IoT-Integrator
pipeline-vulnerabilities discussion (\texttt{paper/main.md} \S 6.5,
\S 7.7, fig 13/14), the full sloppification, model-collapse, and
fabricated-citation evidence base (\texttt{paper/main.md} \S 7), the
redaction-policy register and history-rewrite plan
(\texttt{paper/main.md} \S 5.6; \texttt{docs/redaction-policy.md};
\texttt{docs/git-history-rewrite-plan.md}), the Author's Note
front-matter, the hardware-side effort-gap-compression cluster
(\texttt{paper/main.md} \S 1.4 and \S 6.8), and the
malicious-IoT-integrator and APK-mass-probing scenarios
(\texttt{paper/main.md} \S 7.13 and \S 7.14). A reviewer who needs the
full evidentiary chain should read the long-form companion; this
condensed artifact exists to send them there with informed consent.
