...
- **Upstream identity**: `original/custom_components/powerocean_dev/const.py` line 13 contains `ISSUE_URL = "https://github.com/[REDACTED:repo-path:EF-IMPL-1]/issues"`, indicating the parent integration is `[REDACTED:repo-path:EF-IMPL-1]`. The `noheton/powerocean-dev` repository pinned by the earlier submodule entry was a development fork; the upstream community integration is `[REDACTED:repo-path:EF-IMPL-1]`.
...
1. ~~Pin upstream repository identity~~ — **resolved.** The actual upstream is `[REDACTED:repo-path:EF-IMPL-1]` (per `const.py` `ISSUE_URL`); `noheton/powerocean-dev` is a development fork. Embedded snapshot is at repo commit `ffdf60c`.
