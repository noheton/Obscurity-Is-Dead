...
| T6 | `REPORT.md` §3, §5.3 | **Self-evidencing dataset.** The 906-line probe trace is itself first-class evidence; no code lookup needed. Device serial `80F1B2B3B648` and broker `192.168.1.60:1883` are visible in cleartext within the transcript and should be redacted before any public release. | — |
...
- **`original/doc/log.md`** — community discussion thread documenting the MQTT-broker MITM. Records that the SF cloud broker uses self-signed certificates, the `sf_ggs_cb` username, and the recovered password `euuavURS4Kp9bMUfYmvwl-` (note trailing dash). This is independently strong evidence for the security claim in `REPORT.md` §7 and should be cited directly in the paper.
...
4. Redact the device serial `80F1B2B3B648`, the broker IP `192.168.1.60`, and the recovered MQTT password before any public release.
