...
|---|------|----------|--------|----------|
| 1 | `spiderfarmer_ble` (ESPHome) | C++ | smurfy | ESPHome component for HA |
| 2 | SpiderBLEBridge | C++ / Arduino | p0rigth-dev | ESP32 → MQTT bridge |
| 3 | PythonSpiderController | Python | noheton/community | Standalone Python + MQTT |
...
- `doc/SpiderBLEBridge-master.zip` → `src/main.cpp`, `BLEProtocol.h`, `BLEProtocol_v1.h`
- `doc/PythonSpiderController-main.zip` → `SpiderBLE/`
...
| SpiderBLEBridge | `SECRET_AES_KEY` / `SECRET_AES_IV` from `arduino_secrets.h` (compile-time) |
| PythonSpiderController | Per-device config in `devices.json`; runtime-loaded; supports multi-device |
...
|------|------------------------|
| SpiderBLEBridge | `memcpy(iv, buf+6, 14); iv[14]=0x00; iv[15]=0x00;` |
| PythonSpiderController | `bytearray(packet[6:20]) + b'\x00\x00'` |
...
| Impl | Padding scheme |
|------|---------------|
| SpiderBLEBridge v1 (`BLEProtocol_v1.h`) | PKCS7 (byte value = pad length) |
| SpiderBLEBridge final (`BLEProtocol.h`) | Zero bytes to next 16-byte boundary |
| PythonSpiderController | Zero bytes (skipped if already aligned) |
...
an intermediate SpiderBLEBridge prototype.
...
| PythonSpiderController | Inline buffer accumulation within `send_with_retry()`; no explicit fragment merging — decrypts each fragment independently |
| SpiderBLEBridge | Fixed `rxBuffer`; accumulates BLE notification chunks; parses full packet when size known |
...
|------|--------------------------|
| SpiderBLEBridge | Yes — total_len includes footer CRC |
| PythonSpiderController | Yes — `total_size = payload_len + 8` (6 outer + 2 CRC) |
...
| Impl | Strategy |
|------|----------|
| SpiderBLEBridge | No correlation — fire-and-wait with fixed timeout |
| PythonSpiderController | Stores `last_sent_id` from outgoing JSON; matches against `msgId` in received JSON |
...
| Impl | Step sequence |
|------|---------------|
| SpiderBLEBridge | getSysSta → setDevTimezone → setDevActive |
| PythonSpiderController | **getDevSta** → setDevTimezone → setDevActive *(differs!)* |
| HA BLE | getSysSta → setDevTimezone → setDevActive |
| ESPHome | getSysSta → setDevTimezone → setDevActive |

PythonSpiderController starts with `getDevSta` instead of `getSysSta`. Both
appear to work in practice; `getSysSta` is used by the majority.

### 8. Sensor / Data Coverage

| Sensor | ESPHome | SpiderBLEBridge | PythonSpiderController | HA BLE |
|--------|---------|-----------------|------------------------|--------|
...
|------|---------------------------|
| ESPHome | Partial — outlet switch being added (per smurfy) |
| SpiderBLEBridge | Only heartbeat/keep-alive; no control |
| PythonSpiderController | Framework ready (`setLight`, `setFan`, etc. in BLEPayloads); not wired |
