...
| LED Lamp (FW ≥ 1.7) | `BkJu61kLt3afuogT` | `2AKVNUbU4mvU3Elt` | p0rigth-dev firmware dump + smurfy confirmation |
| Power Strip 10 (FW 3.14) | `lVIlATSlxaS1btfd` | `84Rf7SUkinfvxNlc` | smurfy live decryption confirmed |

**Note:** LED lamps with FW < 1.7 used unencrypted BLE (msgtype 0x0001).
Encryption was introduced in FW 1.7.

**Full candidate list extracted from APK (`libapp.so`) by smurfy:**
...
This matches SpiderBLEBridge `main.cpp`:
```c
