...
|---|---|---|---|---|---|---|---|
| ES-6 | `[REDACTED:repo-path:BALBOA-UPSTREAM-2]` (Python library) | `https://github.com/[REDACTED:repo-path:BALBOA-UPSTREAM-2]`, PyPI `controlmyspa` `[lit-retrieved 2026-05-02]` | MIT | v4.0.0, 2025-08-23 | full cloud API: read state, set temperature, jets, lights, schedules | **Yes.** Calls `iot.controlmyspa.com` with email + password. | **Known issue:** TLS chain at `iot.controlmyspa.com` has been broken since June 2023 (DigiCert intermediate not served by the host and missing from `certifi`); workarounds documented in the README. This is itself a vendor obscurity-vs-security signal (see §1.2.3). |
| ES-7 | `[REDACTED:repo-path:BALBOA-UPSTREAM-1]` | `https://github.com/[REDACTED:repo-path:BALBOA-UPSTREAM-1]` `[lit-retrieved 2026-05-02]` | GPL-3.0 | 73 commits; no tagged release; tested on Novitek spa with Balboa jets | MQTT bridge — sensors, switches, climate; jets HIGH/OFF only (per README) | **Yes** (`CONTROLMYSPA_USER` / `CONTROLMYSPA_PASS` env vars) | Forks an upstream `ControlMySpaJs` Node library; routes via the same cloud as ES-6. |
...
| Cloud host | `iot.controlmyspa.com` (production); also `controlmyspa.com` for marketing | open-source library `[REDACTED:repo-path:BALBOA-UPSTREAM-2]` README and PyPI listing `[lit-retrieved 2026-05-02]` |
...
- ES-6 source tree (`[REDACTED:repo-path:BALBOA-UPSTREAM-2]`).
- ES-7 source tree (`[REDACTED:repo-path:BALBOA-UPSTREAM-1]`) and the
