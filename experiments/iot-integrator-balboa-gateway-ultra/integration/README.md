...
| Backing library | `controlmyspa` 4.0.0 (PyPI, MIT, `arska/controlmyspa`). Used directly by the smoke test; recommended underneath any researcher-supplied HA glue. |
| Recommended HA runtime | `mikakoivisto/controlmyspa-ha-mqtt` (GPL-3.0, MQTT bridge). It already publishes climate + sensors + switches via MQTT auto-discovery. The hardening overlay in `operational-notes.md` is what this folder adds. |
...
- ES-6 source: `https://github.com/arska/controlmyspa`, PyPI `controlmyspa==4.0.0`, MIT.
- ES-7 source: `https://github.com/mikakoivisto/controlmyspa-ha-mqtt`, GPL-3.0.
