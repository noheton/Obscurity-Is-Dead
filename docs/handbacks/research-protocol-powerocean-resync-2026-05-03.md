...
- **Codeowner:** `@[REDACTED:maintainer-handle:EF-IMPL-1]` (unchanged). `documentation` and `issue_tracker` still point at `[REDACTED:repo-path:EF-IMPL-1]` (unchanged).
...
- **Recent commit cadence (atom feed):** dense activity 2026-05-01 → 2026-05-03 (≥20 commits in ~48 h). Notable titles:
  ...
  - `1d1527c565` 2026-05-02 *docs: add [REDACTED:repo-path:EF-IMPL-1] as primary inspiration source*
...
`docs: add [REDACTED:repo-path:EF-IMPL-1] as primary inspiration source` — upstream now formally credits `[REDACTED:repo-path:EF-IMPL-1]` as the parent. The in-repo snapshot's `provenance.md` already established this via `const.py:13 ISSUE_URL`. Confirms (does not change) the existing paper sourcing.
...
- **Material changes detected:** (1) new `DISCLAIMER.md` reframing the integration's legal method; (2) `doc/` redaction with explicit `git rm`-doesn't-purge-history caveat — a real-world rule-12 / rule-13 precedent; (3) OCPP scope (~+440 LOC across `api.py`, `__init__.py`, `services.yaml`, `const.py`) with documented runtime-handover gap; (4) sensor / parser fixes; (5) sourcing acknowledgement of `[REDACTED:repo-path:EF-IMPL-1]`.
