...
- **History rewrite required before public mirror: YES.** Patterns:
  raw maintainer handles (`smurfy`, `p0rigth-dev`, `niltrip`,
  `mikakoivisto`, `arska`), repo paths (`smurfy/esphome-spiderfarmer_ble-encrypt`,
  `p0rigth-dev/SpiderBLEBridge`, `PythonSpiderController`, `niltrip/powerocean`,
  `mikakoivisto/controlmyspa-ha-mqtt`, `arska/controlmyspa`), and the
...
- **Pattern matched / verbatim excerpt:** Real serial numbers
  `HJ37ZDH5ZG5W0109`, `HJ3AZDH5ZG3G0384`, `HJ3AZDH5ZG3G0490`,
  `AC31ZEH4AG130052` co-located with the householder name `Florian Krebs`
...
- **Pattern matched:** `curl http://192.168.1.60:18083/api/v5/clients`
  and same path with `/<clientid>/subscriptions`. `192.168.1.60:18083`
...
### R-AUDIT-05 — HIGH — Repo-display strings `SpiderBLEBridge` and `PythonSpiderController` rendered unredacted in paper text/tables

- **Files / lines:**
  - `paper/main.md:193` (table header column),
  - `paper/main.md:203` (in-prose mention "PythonSpiderController uses `getDevSta`"),
  - `paper/main.tex:730` (table header column),
  - `paper/main.tex` table-body strings within FIG/table macros.
- **Pattern matched:** Verbatim repo names `SpiderBLEBridge` (= the
  `p0rigth-dev/SpiderBLEBridge` second component, which the policy
  redacts to `[REDACTED:repo-path:SF-IMPL-2]`) and `PythonSpiderController`
...
### R-AUDIT-06 — HIGH — `arska/controlmyspa` raw repo path leaked in handback (in-tree, not paper)

- **File / line:** `docs/handbacks/readability-to-writer.md:200`
- **Pattern:** `ES-6 = "the sixth catalogued external-solution entry, \`arska/controlmyspa\`"`
...
- **File / lines (sample, not exhaustive):**
  - `docs/logbook.md:343` (`niltrip/powerocean`),
  - `docs/logbook.md:638-645` (full mapping table: `smurfy/esphome-spiderfarmer_ble-encrypt`,
    `p0rigth-dev/SpiderBLEBridge`, `niltrip/powerocean`),
  - `docs/logbook.md:670,683` (PythonSpiderController),
  - `docs/logbook.md:1048` (`SpiderBLEBridge, PythonSpiderController`),
  - `docs/logbook.md:1318-1363` (`arska/controlmyspa`, `iot.controlmyspa.com`),
  - `docs/logbook.md:1881` (`mikakoivisto/controlmyspa-ha-mqtt`).
...
- **Files / lines:**
  ...
  - `paper/figures/dlr-design-system/project/ui_kits/python_plots/UPSTREAM_README.md:38`
    — `+49 551 7093106 | jan.wagner@dlr.de` (third-party DLR staff phone + email).
...
- **Policy mapping:** Rule 12 implicit ("any information that could
  ...
  `jan.wagner@dlr.de` + landline is **third-party PII** with no consent
  trail in this repo.
- **Recommended action:**
  ...
  - For `jan.wagner@dlr.de` + phone: **redact**. Replace with
...
|----|---------|----------------------------------------|--------------------------|
| H-01 | `smurfy` / `smurfy/esphome-spiderfarmer_ble-encrypt` | `ffdf60c`, `3be5bd8`, prior to `a5c29de` | **Yes** |
| H-02 | `p0rigth-dev` / `p0rigth-dev/SpiderBLEBridge` | `ffdf60c`, `3be5bd8`, `39fb43e`, `6ce1a99`, prior to `a5c29de` | **Yes** |
| H-03 | `pythonspidercontroller` / `PythonSpiderController` | `ffdf60c`, `3be5bd8`, prior to `a5c29de` | **Yes** |
| H-04 | `niltrip` / `niltrip/powerocean` | `ffdf60c` and many subsequent | **Yes** |
| H-05 | `mikakoivisto` / `mikakoivisto/controlmyspa-ha-mqtt` | `96e606e`, `634b317`, `a21d3f2`, prior to `a5c29de` | **Yes** |
| H-06 | `arska` / `arska/controlmyspa` | `4995c6d`, `e9ff2f0`, `96e606e`, `634b317`, prior to `a5c29de` | **Yes** |
...
| H-09 | EcoFlow real serials `HJ37ZDH5ZG5W0109`, `HJ3AZDH5ZG3G0384`, `HJ3AZDH5ZG3G0490`, `AC31ZEH4AG130052` | `ffdf60c` (vendor `original/` import) and any commit touching `equipment.md`/`implementation.md`/`switch.py` since | **Yes** if R-AUDIT-01 is acted on (currently NOT on the policy register — gap) |
| H-10 | `jan.wagner@dlr.de` + `+49 551 7093106` | First commit that imported `paper/figures/dlr-design-system/` (`d57f9c4` design-system curate) | **Yes** if R-AUDIT-08 is acted on |
...
1. **Add EcoFlow PowerOcean serials to the redaction register.**
   Create `R-EF-1..R-EF-4` entries for `HJ37ZDH5ZG5W0109`,
   `HJ3AZDH5ZG3G0384`, `HJ3AZDH5ZG3G0490`, `AC31ZEH4AG130052`. Decide
...
3. **Resolve R-AUDIT-05 (paper text leaks `SpiderBLEBridge` /
   `PythonSpiderController`).** This is the highest-impact in-tree miss
   because it directly contradicts the policy that just paid the
   readability cost of the SF-IMPL-2/3 markers.
4. **Resolve R-AUDIT-06 (`arska/controlmyspa` in handback).** One-line fix.
...
6. **Decide on third-party DLR contact info** (R-AUDIT-08). Recommend
   redact `jan.wagner@dlr.de` + phone unconditionally; verify-with-author
