...
| P3a | Layout hand-back from Stage 4 (LAY-32 H + LAY-31 M + LAY-33 L) | open | LAY-32 (H, illustrator): regenerate `fig15-apk-mass-probing.pdf` to remove the dark-fill stage-box header / body-label collision. LAY-31 (M, writer): wrap `\texttt{noheton/spider\_farmer}` + `\texttt{noheton/powerocean-dev}` in `\seqsplit{}` or split the §5.6 trailing clause into a separate sentence (joint-closable with RDB-39). LAY-33 (L, writer advisory): consider `\FloatBarrier` after §7.13 / §7.14 / §7.15 to relieve the four-page float cascade, or accept the 3-page inflation. After the next writer + illustrator pass + `make pdf` rebuild, re-dispatch P3 for re-scrutiny. |
