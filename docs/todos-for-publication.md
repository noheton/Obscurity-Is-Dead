...
| P3a | Layout hand-back from Stage 4 (LAY-32 H + LAY-31 M + LAY-33 L) | in-progress | LAY-32 (H, illustrator) — **done** 2026-05-04: `paper/figures/fig15-apk-mass-probing.{py,svg,pdf}` regenerated; dark-fill stage-box header collapsed 3-lines → 2-lines (break inside `identity-provider`) + header font 9.4 → 9.0 pt (still above the 9 pt body floor); overlap resolved at 220 dpi. LAY-31 (M, writer) — open: wrap `\texttt{noheton/spider\_farmer}` + `\texttt{noheton/powerocean-dev}` in `\seqsplit{}` or split the §5.6 trailing clause into a separate sentence (joint-closable with RDB-39). LAY-33 (L, writer advisory) — open: consider `\FloatBarrier` after §7.13 / §7.14 / §7.15 to relieve the four-page float cascade, or accept the 3-page inflation. After the writer pass + `make pdf` rebuild, re-dispatch P3 for re-scrutiny. |
